...
<main class="flex-1 flex items-center justify-center p-8 overflow-y-auto bg-white">
  <div class="w-full max-w-lg bg-white border border-gray-200 rounded-2xl shadow-lg p-8">
    <div class="mb-6 text-center">
      ...
      <p class="text-gray-500 text-sm">Perbarui data prodi yang diinginkan</p>
    </div>

    <form action="{{ route('prodi.update', $prodi['id_prodi']) }}" method="POST" class="space-y-5">
      @csrf
      @method('PUT')

      <div>
        <label class="block text-sm font-semibold text-gray-700 mb-1">ID Prodi</label>
        <input type="text" name="id_prodi" value="{{ old('id_prodi', $prodi['id_prodi']) }}" placeholder="Masukkan ID"
      ...
      <div>
        ...
        <input type="text" name="nama_prodi" value="{{ old('nama_prodi', $prodi['nama_prodi']) }}" placeholder="Masukkan Nama"
