...
<body class="bg-gradient-to-tr from-blue-50 via-white to-green-50 font-[system-ui] leading-normal tracking-normal">
  ...
  <div class="w-full max-w-lg bg-white border border-gray-200 rounded-2xl shadow-lg p-8">
    <div class="mb-6 text-center">
      ...
      <p class="text-gray-500 text-sm">Perbarui data mahasiswa yang dipilih</p>
    </div>
    ...
    <form action="{{ route('mahasiswa.update', $mahasiswa['npm']) }}" method="POST" class="space-y-5">
      ...
      <div>
        ...
        <input type="text" name="npm" value="{{ $mahasiswa['npm'] }}" readonly
          class="w-full bg-gray-100 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none">
      </div>
      ...
      <div class="mb-4">
        ...
        <select name="kode_kelas" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 transition" required>
          <option value="">-- Pilih Kelas --</option>
          @foreach ($kelas as $k)
            <option value="{{ $k['kode_kelas'] }}" {{ $mahasiswa['kode_kelas'] == $k['kode_kelas'] ? 'selected' : '' }}>
              {{ $k['nama_kelas'] }}
            </option>
          @endforeach
        </select>
      ...
      <div class="mb-4">
        <label class="block text-sm font-semibold text-gray-700 mb-1">ID Prodi</label>
        <select name="id_prodi" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 transition" required>
