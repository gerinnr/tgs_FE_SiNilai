    ...
    <div class="p-6">
      ...
      <main class="flex-1 p-8 overflow-y-auto bg-white border-l border-gray-300">
        ...
        <div class="mb-6">
          ...
          <p class="text-gray-500">Berikut adalah data kelas yang tersedia dalam sistem.</p>
        </div>

        <!-- Header Tabel -->
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
          <div>
            <h3 class="text-xl font-semibold text-gray-800">Daftar Kelas</h3>
            <p class="text-sm text-gray-500">Kelola data kelas yang tersedia di sistem.</p>
          </div>

          <div class="flex flex-col md:flex-row items-center gap-2">
            <!-- Form Pencarian -->
            <form method="GET" action="{{ route('kelas.index') }}" class="flex items-center border rounded-md overflow-hidden bg-white shadow-sm">
            ...
            <!-- Tombol Tambah -->
            <a href="{{ route('kelas.create') }}" class="bg-green-500 text-white px-4 py-2 rounded-md text-sm hover:bg-green-700 shadow-sm">
          ...
          <table class="min-w-full table-auto text-sm">
            ...
            <tbody id="kelasTableBody">
              ...
              <tr class="hover:bg-blue-50 border-b">
                ...
                <td class="py-2 px-4 text-center">{{ $kls['kode_kelas'] }}</td>
                <td class="py-2 px-4 text-center">{{ $kls['nama_kelas'] }}</td>
                <td class="py-2 px-4 text-center space-x-1">
                  <a href="{{ route('kelas.edit', $kls['kode_kelas']) }}" class="bg-yellow-400 text-white px-3 py-1 rounded hover:bg-yellow-500 transition">
                    <i class="fas fa-edit"></i>
                  </a>
                  <form action="{{ route('kelas.destroy', $kls['kode_kelas']) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
